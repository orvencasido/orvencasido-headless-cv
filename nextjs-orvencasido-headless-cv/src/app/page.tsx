...
const HeaderSection = ({ resume, profileImageUrl }: { resume: any, profileImageUrl: string | null | undefined }) => {
  return (
...
const ExperienceSection = ({ experience }: { experience?: any[] }) => {
      ...
      <div className="space-y-16 md:space-y-20">
        {skills.map((skillItem: any, idx: number) => {
          ...
          const skillImageUrl = typeof skillItem === 'object' && skillItem?.image
            ? urlFor(skillItem.image)?.width(400).height(400).quality(100).url()
            : null
